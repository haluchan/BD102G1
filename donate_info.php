...
<?php 
	...
	try {
		...
		$sql = "select * from donate_info where event_no = :event_no";
 ...
 ?>
		...
		<div class="food col-sm-12">
			...
			<?php 
				...
				$i = 0;
				while ( $i < $count ) {
					$sql = "select * from plant where plant_no ='$plants[$i]'" ;
					$plent = $pdo ->query($sql);
	...
	<section class="progress_return">
		...
		<?php 
				$sql = "select * from bd102g1.return where event_no =:event_no and return_status = 'Y'" ;
...
<!-- 留言評論======================================== -->
	...
	<?php 
		$sql = "select * from message_event where event_no =:event_no and xreport_ny not in('Y') ORDER BY `msg_date` desc" ;
		$msg = $pdo ->prepare($sql);
