...
$pdo = new PDO( $dsn, $user, $password );
		...
		if($form=="new"){
			try {
				...
				try {
					// require_once("connectPon.php");//之後要換成connectGrowing_hope.php
					$sql="select * from  bd102g1.return join bd102g1.event where bd102g1.event.event_no= ? AND bd102g1.event.event_dept=? ";
  echo isset($pdo) ? "==========yes":"===========no";
					$event=$pdo->prepare($sql);
					...
					if($event->rowCount()===0){
		    			...
		    			$sql = "insert into bd102g1.return (return_no, event_no, return_date, return_info, return_pho) values(:return_no,:event_no, current_date(),:return_info,:return_pho) ";
						...
						$sql="select COUNT(event_no) from bd102g1.return where event_no=$no";
