...
<form class="myForm">
    ...
    $sqlActName = "select act_name from activity";
 	//共有幾筆資料
	$sql = "select count(*) from actSign";
	$result = $pdo->query($sql);
	$result2 = $pdo->query($sqlActName);
	$result->bindColumn(1,$totalRecord);
	$result->fetch();
	$resultset2 =  $result2->fetchObject();
	...
	$sql = "select * from actSign,activity where activity.act_no = actSign.act_no order by actSign_no limit $start,$recPerPage ";
...
?>
		<tr class="tdRow">
			...
			<td><?php echo $resultset->act_name ;?></td>
